...
#pragma mark ----------wgs84转换其他坐标系
+ (CLLocationCoordinate2D)sb_wgs84_to_gcj02:(CLLocationCoordinate2D)location{
    return [SBTools gcj02Encrypt:location.latitude bdLon:location.longitude];
}
...
+ (CLLocationCoordinate2D)sb_wgs84_to_bd09:(CLLocationCoordinate2D)location{
    ...
    return [SBTools bd09Encrypt:gcj02Pt.latitude bdLon:gcj02Pt.longitude] ;
}


#pragma mark ----------gcj02转换其他坐标系
+ (CLLocationCoordinate2D)sb_gcj02_to_wgs84:(CLLocationCoordinate2D)location
{
    return [SBTools gcj02Decrypt:location.latitude gjLon:location.longitude];
}

+ (CLLocationCoordinate2D)sb_gcj02_to_bd09:(CLLocationCoordinate2D)location
{
    return  [SBTools bd09Encrypt:location.latitude bdLon:location.longitude];
}


#pragma mark ----------bd09转换其他坐标系
...
{
    CLLocationCoordinate2D gcj02 = [SBTools sb_bd09_to_gcj02:location];
...
+ (void) openUrlApplication:(SBMapApplicationType) type locationCoordinate2D:(CLLocationCoordinate2D)locationCoordinate name:(NSString *)name{
    ...
    if (type == SBMapApplicationTypeApple) {
        ...
        MKMapItem *tolocation = [[MKMapItem alloc] initWithPlacemark:[[MKPlacemark alloc] initWithCoordinate:[SBTools sb_bd09_to_gcj02:locationCoordinate] addressDictionary:nil]];
        tolocation.name=name;
        [MKMapItem openMapsWithItems:@[currentLocation,tolocation]launchOptions:@{MKLaunchOptionsDirectionsModeKey:MKLaunchOptionsDirectionsModeDriving,MKLaunchOptionsShowsTrafficKey:[NSNumber numberWithBool:YES]}];
    }else if (type == SBMapApplicationTypeGaode){
        urlsting =[[NSString stringWithFormat:@"iosamap://path?sourceApplication=applicationName&sid=BGVIS1&did=BGVIS2&dlat=%lf&dlon=%lf&dname=%@&dev=0&t=0",[SBTools sb_bd09_to_gcj02:locationCoordinate].latitude,[SBTools sb_bd09_to_gcj02:locationCoordinate].longitude,name] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
