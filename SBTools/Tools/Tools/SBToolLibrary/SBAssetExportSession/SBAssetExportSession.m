...
@implementation AVAsset (SBAssetDegress)

/// 获取视频角度
- (int)sb_degressFromVideo {
    int degress = 0;
...
inline static CGSize sb_assetExportSessionPresetSize(SBAssetExportSessionPreset preset)
{
    CGSize size;
    switch (preset) {
        case SBAssetExportSessionPreset240P:
            size = CGSizeMake(240, 360);
            break;
        case SBAssetExportSessionPreset360P:
            size = CGSizeMake(360, 480);
            break;
        case SBAssetExportSessionPreset480P:
            size = CGSizeMake(480, 640);
            break;
        case SBAssetExportSessionPreset540P:
            size = CGSizeMake(540, 960);
            break;
        case SBAssetExportSessionPreset1080P:
            size = CGSizeMake(1080, 1920);
            break;
        case SBAssetExportSessionPreset2K:
            size = CGSizeMake(1440, 2560);
            break;
        case SBAssetExportSessionPreset4K:
            size = CGSizeMake(2160, 3840);
            break;
        case SBAssetExportSessionPreset720P:
        default:
...
inline static SBAssetExportSessionPreset sb_assetExportSessionPresetFromSize(CGSize size)
{
    ...
    if (size.width <= 240 && size.height <= 360) {
        return SBAssetExportSessionPreset240P;
    }
    if (size.width <= 360 && size.height <= 480) {
        return SBAssetExportSessionPreset360P;
    }
    if (size.width <= 480 && size.height <= 640) {
        return SBAssetExportSessionPreset480P;
    }
    if (size.width <= 540 && size.height <= 960) {
        return SBAssetExportSessionPreset540P;
    }
    if (size.width <= 720 && size.height <= 1280) {
        return SBAssetExportSessionPreset720P;
    }
    if (size.width <= 1080 && size.height <= 1920) {
        return SBAssetExportSessionPreset1080P;
    }
    if (size.width <= 1440 && size.height <= 2560) {
        return SBAssetExportSessionPreset2K;
    }
    if (size.width <= 2160 && size.height <= 3840) {
        return SBAssetExportSessionPreset4K;
    }
    return SBAssetExportSessionPreset240P;
}

inline static unsigned long sb_assetExportSessionPresetBitrate(SBAssetExportSessionPreset preset)
{
...
inline static NSDictionary *sb_assetExportVideoConfig(CGSize size, SBAssetExportSessionPreset preset)
{
    float ratio = 1;
    CGSize presetSize = sb_assetExportSessionPresetSize(preset);
    ...
    SBAssetExportSessionPreset realPreset = sb_assetExportSessionPresetFromSize(videoSize);
    unsigned long bitrate = sb_assetExportSessionPresetBitrate(realPreset);
...
inline static NSDictionary *sb_assetExportVideoOutputConfig(void)
{
    return @{
...
inline static NSDictionary *sb_assetExportAudioOutputConfig(void)
{
    return @{
...
@interface SBAssetExportSession ()
{
...
@property (nonatomic, assign) SBAssetExportSessionPreset preset;
...
@implementation SBAssetExportSession
{
    NSError *_error;
...
- (instancetype)initWithAsset:(AVAsset *)asset preset:(SBAssetExportSessionPreset)preset
{
    if ((self = [self init]))
    {
        _preset = preset;
        _asset = asset;
        _videoOrientation = [asset sb_degressFromVideo];
    }
...
- (float)estimatedExportSize
{
    ...
    if (self.videoSettings) {
        ...
        if (videoTracks.count > 0) {
            ...
            videoBitrate = [[[sb_assetExportVideoConfig(videoTrack.naturalSize, self.preset) objectForKey:AVVideoCompressionPropertiesKey] objectForKey:AVVideoAverageBitRateKey] unsignedLongValue];
        }
    }
...
- (void)_setupAudioUsingTracks:(NSArray *)audioTracks {
    ...
    if (audioTracks.count > 0) {
        NSDictionary *settings = sb_assetExportAudioOutputConfig();
...
- (void)_setupVideoUsingTracks:(NSArray *)videoTracks {
    ...
    if (videoTracks.count > 0) {
        ...
        if (settings == nil) {
            settings = sb_assetExportVideoOutputConfig();
        }
    ...
    if (self.videoOutput) {
        ...
        if (videoSettings == nil) {
            videoSettings = sb_assetExportVideoConfig(_inputBufferSize, self.preset);
        }
...
- (void)_setProgress:(float)progress {
    ...
    void (^doProgress)(void) = ^{
        ...
        id<SBAssetExportSessionDelegate> delegate = self.delegate;
